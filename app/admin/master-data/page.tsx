...
import { MasterData, defaultMasterData, MASTER_DATA_VERSION } from '@/types/master-data';
...
export default function MasterDataPage() {
  ...
  useEffect(() => {
    let data = defaultMasterData;
    const savedVersion = localStorage.getItem('masterDataVersion');

    // ถ้า version ไม่ตรง ให้ reset เป็น default
    if (savedVersion !== String(MASTER_DATA_VERSION)) {
      localStorage.setItem('masterData', JSON.stringify(defaultMasterData));
      localStorage.setItem('masterDataVersion', String(MASTER_DATA_VERSION));
    } else {
    ...
    <AdminLayout>
      <div className="p-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 mt-1">Manage all reusable option sets used across the system</p>
        </div>
              ...
              {paged.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-gray-500">
                    {searchTerm || statusFilter !== 'all' ? 'ไม่พบรายการที่ค้นหา' : 'ยังไม่มี Master Data ในระบบ'}
                  </td>
                </tr>
              ) : (
                paged.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/admin/master-data/${item.id}?mode=view`}
                        ...
                        <Link
                          href={`/admin/master-data/${item.id}?mode=edit`}
                          className="p-2 text-gray-400 hover:text-gray-700 transition-colors"
                          title="Edit"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
