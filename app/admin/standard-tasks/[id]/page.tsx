    ...
    <div className="border border-gray-200 rounded-lg p-4">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        <div className="md:col-span-4">
          ...
          <input type="text" value={field.label} onChange={(e) => onChange({ ...field, label: e.target.value })} disabled={disabled}
            className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 focus:border-transparent disabled:bg-gray-50 disabled:text-gray-600" placeholder="e.g. Oil Pressure Reading" />
        </div>
        <div className="md:col-span-3">
          <label className="block text-sm font-medium text-gray-600 mb-1">Field Type</label>
          <select value={field.type} onChange={(e) => onChange({ ...field, type: e.target.value as FieldType })} disabled={disabled}
        ...
        <div className="md:col-span-2 flex items-end pb-1">
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={field.required} onChange={(e) => onChange({ ...field, required: e.target.checked })} disabled={disabled}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3 pt-3 border-t border-gray-100">
          <div>
            ...
            <input type="text" value={field.unit || ''} onChange={(e) => onChange({ ...field, unit: e.target.value })} disabled={disabled}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 disabled:bg-gray-50 disabled:text-gray-600" placeholder="e.g. PSI" />
          </div>
          <div>
...
export default function StandardTaskDetailPage() {
  ...
  const [form, setForm] = useState<StandardTask>({
    id: '', name: '', description: '', category: 'cleaning', status: 'active', fields: [],
    updatedAt: new Date().toISOString().split('T')[0], updatedBy: 'Admin User',
  });

  useEffect(() => {
    let allData = defaultStandardTasks;
    const saved = localStorage.getItem('standardTasks');
    if (saved) {
      try { allData = JSON.parse(saved); } catch (e) { console.error('Failed to parse:', e); }
    }
    const found = allData.find(item => item.id === id);
    startTransition(() => {
      if (found) setForm({ ...found, fields: found.fields || [] });
      setIsLoaded(true);
    });
  }, [id]);
  ...
  const handleAddField = () => {
    const newField: TaskField = { id: crypto.randomUUID().substring(0, 8), label: '', type: 'checkbox', required: false };
  ...
  const modeLabel = isEditing ? 'Edit Task' : 'View Task';
    ...
    <AdminLayout>
      <div className="p-6 max-w-5xl">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              {isEditing ? 'Update the task definition and its input fields' : 'View the task definition and its input fields'}
            </p>
          </div>
        ...
        <div className="bg-white rounded-lg shadow mb-6">
          <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">{isEditing ? 'Update task details' : 'Task details'}</p>
            </div>
          ...
          <div className="p-6 space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value as TaskCategory })} disabled={!isEditing}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm bg-white focus:ring-2 focus:ring-teal-500 disabled:bg-gray-50 disabled:text-gray-600">
                ...
                <div>
                  ...
                  <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as 'active' | 'inactive' })} disabled={!isEditing}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm bg-white focus:ring-2 focus:ring-teal-500 disabled:bg-gray-50 disabled:text-gray-600">
            ...
            <div>
              ...
              <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} disabled={!isEditing} rows={3}
        ...
        <div className="bg-white rounded-lg shadow mb-6">
          <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Task Fields</h2>
              <p className="text-sm text-gray-500">Input fields crew will fill when completing this task</p>
            </div>
            {isEditing && (
          ...
          <div className="p-6">
            {form.fields.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                {isEditing && <button onClick={handleAddField} className="text-teal-700 hover:text-teal-800 text-sm font-medium">+ Add first field</button>}
              </div>
            ) : (
              <div className="space-y-4">
                {form.fields.map((field, idx) => (
                  <div key={field.id} className="relative">
                    ...
                    <FieldConfigPanel field={field} onChange={(f) => handleFieldChange(field.id, f)} onRemove={() => handleRemoveField(field.id)} disabled={!isEditing} />
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="flex items-center gap-3">
          {isEditing ? (
            <>
              ...
              <button onClick={() => { setIsEditing(false); router.push(`/admin/standard-tasks/${id}?mode=view`); }} className="px-6 py-2.5 bg-gray-200 text-gray-700 rounded-lg text-sm hover:bg-gray-300 transition-colors font-medium">Cancel</button>
            </>
          ) : (
            <button onClick={() => setIsEditing(true)} className="px-6 py-2.5 bg-teal-700 text-white rounded-lg text-sm hover:bg-teal-800 transition-colors font-medium">Edit Task</button>
          )}
        </div>
      </div>
    </AdminLayout>
