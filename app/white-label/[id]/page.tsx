...
export default function WhiteLabelDetailPage() {
  ...
  const handleTierChange = (newTier: 'basic' | 'professional' | 'enterprise') => {
    ...
    updateConfig({ tier: newTier, enabledFeatures: updatedEnabledFeatures });
  };

  const handleSave = () => {
    if (!config) return;
    localStorage.setItem(`whiteLabel_${id}`, JSON.stringify(config));
    const items: WhiteLabelItem[] = JSON.parse(localStorage.getItem('whiteLabelItems') || '[]');
    ...
    const item = {
      ...
      status: existingIndex >= 0 ? items[existingIndex].status : 'active' as const,
    };
    if (existingIndex >= 0) {
        ...
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm mt-1">Customize the look and feel of your platform for your clients</p>
            </div>
          </div>
          <button
            onClick={handleSave}
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
              <ColorPicker
                ...
                onChange={(v) => updateConfig({ colors: { ...config.colors, primary: v } })}
              />
              <ColorPicker
                label="Secondary Color"
                value={config.colors.secondary}
                onChange={(v) => updateConfig({ colors: { ...config.colors, secondary: v } })}
              />
              <ColorPicker
                label="Accent Color"
                value={config.colors.accent}
                onChange={(v) => updateConfig({ colors: { ...config.colors, accent: v } })}
              />
              <ColorPicker
                label="Background Color"
                value={config.colors.background}
                onChange={(v) => updateConfig({ colors: { ...config.colors, background: v } })}
              />
              <ColorPicker
                label="Text Color"
                value={config.colors.text}
                onChange={(v) => updateConfig({ colors: { ...config.colors, text: v } })}
              />
            </div>
            ...
            <div className="mt-8 p-5 rounded-lg border border-gray-200 bg-gray-50">
              ...
              <div className="flex flex-wrap gap-3">
                <button style={{ backgroundColor: config.colors.primary }} className="px-4 py-2 text-white rounded-lg text-sm">
                  Primary
                </button>
                <button style={{ backgroundColor: config.colors.secondary }} className="px-4 py-2 text-white rounded-lg text-sm">
                  Secondary
                </button>
                <button style={{ backgroundColor: config.colors.accent }} className="px-4 py-2 text-white rounded-lg text-sm">
                  Accent
                </button>
              </div>
            </div>
